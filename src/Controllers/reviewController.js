...
const deleteReview = async function (req, res) {
    try {
        ...
        if (!ObjectId.isValid(reviewIdParams))
            return res.status(400).send({ status: false, message: `${reviewIdParams} is not a valid review id` })
       ...
       // Finding not deleted review by id in url
        ...
        // Deleting the review
        await reviewModel.findOneAndUpdate({ _id: reviewIdParams }, { isDeleted: true })
        ...
        return res.status(200).send({ status: true, message: `${reviewIdParams} this review is deleted successfully` })
...
//Exporting Modules
module.exports.createReview = {createReview,deleteReview,updateReview}
